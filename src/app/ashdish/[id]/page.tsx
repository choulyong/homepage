/**
 * AshDish Post Detail Page - AshDish 상세보기
 */
...
export default function AshDishDetailPage({ params }: PageProps) {
  const { id } = use(params);
  const router = useRouter();
  const [post, setPost] = useState<AshDishPost | null>(null);
  ...
  const loadUser = () => {
    ...
    console.log('🔐 AshDish Detail - Cookie user:', currentUser);
    setUser(currentUser);
  };
  ...
  const loadPost = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/ashdish/${id}`);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/ashdish/${post.id}/like`, {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/ashdish?id=${encodeURIComponent(post.id)}`, {
        method: 'DELETE'
      });
  ...
  const loadComments = async () => {
    try {
      const response = await fetch(`/api/ashdish/${id}/comments`);
      const data = await response.json();
  ...
  const handleCommentDelete = async (commentId: string) => {
    ...
    try {
      const response = await fetch(`/api/ashdish/${id}/comments?commentId=${commentId}`, {
        method: 'DELETE',
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/ashdish"><button className="px-6 py-3 bg-red-500 hover:bg-red-600 text-white rounded-lg">목록으로</button></Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <Link href="/ashdish" className="text-red-600 hover:text-red-500 mb-6 inline-block">← 목록으로</Link>
