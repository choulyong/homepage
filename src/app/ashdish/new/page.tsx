/**
 * AshDish New Post Page - AshDish 글쓰기
 */
...
export default function AshDishNewPage() {
  ...
  const checkAuth = async () => {
    try {
      ...
      console.log('🔐 AshDish New - 사용자 확인:', currentUser);

      if (!currentUser) {
        alert('로그인이 필요합니다. 로그인 페이지로 이동합니다.');
        // 현재 페이지를 redirect 파라미터로 전달
        router.replace('/auth/login?redirect=/ashdish/new');
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (videoType === 'upload' && videoFile) {
        ...
        formData.append('bucket', 'ashdish-videos');
      ...
      if (imageFiles.length > 0) {
        for (let i = 0; i < imageFiles.length; i++) {
          ...
          formData.append('bucket', 'ashdish-images');
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/ashdish" className="text-red-600 hover:text-red-500 mb-4 inline-block">
            ← 목록으로
          </Link>
